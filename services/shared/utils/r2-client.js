// ============================================================
// 🧠 AI Podcast Suite — Unified Cloudflare R2 Client + Helpers
// ============================================================
//
// Provides a centralized interface for interacting with R2 buckets
// (upload, download, list, and public URL utilities) using the AWS S3 SDK.
//
// ------------------------------------------------------------
...
export async function getR2ReadStream(bucket, key) {
  try {
    ...
    info("r2.get.readStream.success", { bucket, key });
    return response.Body;
  } catch (err) {
    error("r2.get.readStream.fail", { bucket, key, error: err.message });
    throw err;
  }
}
...
export async function putObject(params) {
  try {
    ...
    info("r2.put.object.success", { bucket: params.Bucket, key: params.Key });
  } catch (err) {
    error("r2.put.object.fail", {
...
export async function getObject(bucket, key) {
  try {
    ...
    info("r2.get.object.success", { bucket, key, size: buf.length });
    return buf;
  } catch (err) {
    error("r2.get.object.fail", { bucket, key, error: err.message });
    throw err;
  }
}
...
export async function getObjectAsText(bucket, key) {
  try {
    ...
    info("r2.get.objectAsText.success", { bucket, key, length: text?.length });
    return text;
  } catch (err) {
    error("r2.get.objectAsText.fail", { bucket, key, error: err.message });
    throw err;
  }
}
...
export function buildPublicUrl(bucket, key) {
  ...
  return `${base.replace(/\\/+/g, "/").replace(/\\/$/, "")}/${String(
    key,
  ).replace(/^\\/+/, "")}`;
      }
